...
public class DaoCarDriver extends Dao {
    ...
    public List<Car> selectBrokenCars() {
        ...
        try {
            ...
            var query = criteria.createQuery(Car.class);
            var selection = query.from(Car.class);
            query.where(criteria.equal(selection.get(Car_.state), 0));
            cars = manager.createQuery(query).getResultList();
